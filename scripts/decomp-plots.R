...
## DWS: note that plots and models below have problem: spread rate does not
## vary within species.  So pseudoreplicated.
...
## DWS: why are there hard-coded coefficients, below. use predict()!
...
new.data$spread.pred <- (-0.6192 + 0.01405*(new.data$l.mean) -0.00005043*((new.data$l.mean)^2))
new.data$spread.pred[new.data$spread.pred < 0] = 0
