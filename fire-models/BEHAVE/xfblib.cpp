            ...
            double *flatDistance )
{
      ...
      && flameHt > SMIDGEN )
    {
        ...
        {
            // Flat terrain spotting distance.
            flatDist = 0.000718 * windSpeedAt20Ft * sqrt( ht )
                     * ( 0.362 + sqrt( z / ht ) / 2.
                     * log( z / ht ) );
...
double FBL_SurfaceFireEllipsePsiDegrees( double f, double h, double thetaDegrees )
{
    double thetaRadians = FBL_CompassDegreesToRadians(
                            FBL_CompassConstrainDegrees( thetaDegrees ) );
    double psiRadians   = FBL_SurfaceFireEllipsePsiRadians( f, h, thetaRadians );
...
double FBL_SurfaceFireEllipsePsiRadians( double f, double h, double thetaRadians )
{
...
//------------------------------------------------------------------------------
/*! \brief Calculates the fire ellipse angle 'theta' in degrees.
 *
 *  \param f Ellipse parameter f (half the length of the major axis)
 *           See Catchpole, de Mestre, and Gill (1982).
 ...
 */
double FBL_SurfaceFireEllipseThetaDegrees( double f, double g, double h,
        double betaDegrees )
{
    double betaRadians  = FBL_CompassDegreesToRadians(
                            FBL_CompassConstrainDegrees( betaDegrees ) );
    double thetaRadians = FBL_SurfaceFireEllipseThetaRadians( f, g, h,
                            betaRadians );
...
//------------------------------------------------------------------------------
/*! \brief Calculates the fire ellipse angle 'theta' in radians.
 *
 *  \param f Ellipse parameter f (half the length of the major axis)
 *           See Catchpole, de Mestre, and Gill (1982).
 *  \param g Ellipse parameter g (half the length of the major axis minus the
 ...
 */
double FBL_SurfaceFireEllipseThetaRadians( double f, double g, double h,
        double betaRadians )
{
...
{
    double thetaDeg = FBL_SurfaceFireEllipseThetaDegrees( f, g, h, betaDegrees );
...
{
    double psiDegrees = FBL_SurfaceFireEllipsePsiDegrees( f, h, thetaDegrees );
    ...
    double crownVolScorched, double scorchHt )
{
    ...
    if ( FofemSpecies[speciesIndex].mortEq == 3 && mr < 0.8 )
    {
        mr = 0.8;
    }
